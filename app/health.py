...
async def check_tnlcm_alive() -> dict[str, Any]:
    ...
    try:
        ...
        return {"alive": True, "url": url, "detail": f"HTTP {response.status_code}"}
    except httpx.TimeoutException:
        return {"alive": False, "url": url, "detail": "timeout"}
    except httpx.HTTPError as exc:
        # ConnectError, ConnectTimeout, etc.: TNLCM no responde.
        return {"alive": False, "url": url, "detail": str(exc) or exc.__class__.__name__}


async def check_services() -> dict[str, Any]:
    """Compone el liveness del orquestador propio y de TNLCM."""
    orchestrator = {"alive": True, "url": None, "detail": "running"}
...
async def probe_http_service(url: str, healthy_statuses: Optional[set[int]]) -> tuple[bool, str]:
...
async def _probe_service(name: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    healthy, detail = await probe_http_service(url, cfg["healthy_statuses"])
