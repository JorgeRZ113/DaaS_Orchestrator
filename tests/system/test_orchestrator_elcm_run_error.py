...
from app.domain.descriptor import DatasetDescriptor, ExperimentConfig, InfrastructureConfig
...
async def test_run_elcm_phase_run_error_is_propagated_without_retry(monkeypatch, caplog):
    ...
    monkeypatch.setattr("app.adapters.elcm.upload_test_cases", _fake_upload_test_cases)
    monkeypatch.setattr("app.adapters.elcm.run_experiment", _fake_run_experiment)
    monkeypatch.setattr("app.adapters.tnlcm.destroy_trial_network", _fake_destroy_trial_network)
    ...
    descriptor = DatasetDescriptor(
        ...
        experiment=ExperimentConfig(name="exp-elcm", testcase_paths=["TC_1_Preflight.yml"]),
    )
    ...
    await elcm_phase.run_elcm_phase(execution_id, descriptor.experiment)
...
async def test_run_elcm_phase_upload_error_stops_before_run_experiment(monkeypatch):
    ...
    async def _fake_upload_test_cases(testcase_paths, elcm_base_url=None, execution_id=None):
        ...
        raise elcm.TnUploadTestCaseError(
            "ELCM upload_test_case failed for TC_1_Preflight.yml (HTTP 400). Backend error: user_id ausente/invalido. Corrija lo indicado por el mensaje de error antes de volver a lanzar la parte de ELCM."
        )
...
async def test_run_elcm_phase_logs_not_found_bypasses_tn_status_check(monkeypatch):
    ...
    monkeypatch.setattr("app.adapters.elcm.upload_test_cases", _fake_upload_test_cases)
    monkeypatch.setattr("app.adapters.elcm.run_experiment", _fake_run_experiment)
    monkeypatch.setattr("app.adapters.elcm.get_experiment_status", _fake_get_experiment_status)
    monkeypatch.setattr("app.adapters.elcm.collect_results", _fake_collect_results)
    monkeypatch.setattr("app.adapters.tnlcm.destroy_trial_network", _fake_destroy_trial_network)
    monkeypatch.setattr("app.adapters.tnlcm.get_tn_status", _fake_get_tn_status)
    monkeypatch.setattr("app.storage.artifacts.build_artifacts", _fake_build_artifacts)
    monkeypatch.setattr("app.adapters.wireguard.down_tunnel", _fake_down_tunnel)
