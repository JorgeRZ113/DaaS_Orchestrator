...
def _isolate_orchestrator_state(monkeypatch, tmp_path):
    ...
    async def _noop_persist(execution_id, stage):
        return None

    monkeypatch.setattr(orchestrator, "_persist_telemetry_report_best_effort", _noop_persist)
...
def _patch_common(monkeypatch, destroy_calls: list[str], state_result):
    ...
    monkeypatch.setattr(orchestrator, "generate_tnlcm_descriptor", _fake_generate)
    monkeypatch.setattr("app.tnlcm.deploy_trial_network", _fake_deploy)
    monkeypatch.setattr("app.tnlcm.get_tn_state", _fake_get_tn_state)
    monkeypatch.setattr("app.tnlcm.destroy_trial_network", _fake_destroy)
...
async def test_tnlcm_phase_failure_keeps_healthy_activated_tn(monkeypatch):
    ...
    await orchestrator.run_tnlcm_phase(execution_id, _descriptor(execution_id))
